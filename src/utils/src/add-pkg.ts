...
async function run() {
  ...
  await mkdir(join(path, 'packages', pkg.name))
  await mkdir(join(path, 'packages', pkg.name, 'src'))
  await Promise.all([
    copyAndTransformFile(
      join(process.cwd(), 'templates', 'package.json'),
      join(path, 'packages', pkg.name, 'package.json'),
      async source => parseTemplate(source, { package: pkg })
    ),
    copyAndTransformFile(
      join(process.cwd(), 'templates', 'tsconfig.json'),
      join(path, 'packages', pkg.name, 'tsconfig.json'),
      async source => parseTemplate(source, { package: pkg })
    ),
    copyAndTransformFile(
      join(process.cwd(), 'templates', 'src/index.ts'),
      join(path, 'packages', pkg.name, 'src/index.ts'),
      async source => parseTemplate(source, { package: pkg })
    ),
  ])

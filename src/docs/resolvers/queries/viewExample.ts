...
import type { Query, QueryViewExampleArgs } from '@browserql/docs-graphql/src/generated/graphql';
import fromMarkdown from 'mdast-util-from-markdown'
import toMarkdown from 'mdast-util-to-markdown'
import YAML from 'yaml'

export async function viewExample({ module: mod, example }: QueryViewExampleArgs): Promise<Query['viewExample']> {
  const path = resolve(
    process.cwd(),
    '../../examples',
    mod,
          ...
          parsed.screens.map(async screen => {
            if (screen.source) {
              return {
                ...screen,
                name: screen.source,
                source: (await readFile(join(path, 'src', screen.source))).toString()
              }
            }
            if (screen.eval) {
